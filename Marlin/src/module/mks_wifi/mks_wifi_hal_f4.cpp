...
#if defined(MKS_WIFI) && defined(STM32F4)

volatile unsigned char *buff;
volatile unsigned char buffer_ready;
volatile unsigned char dma_stopped;

volatile GPIO_TypeDef* esp_io_port;
volatile uint32_t esp_io_en_pin;
...
void mks_wifi_hw_prepare(unsigned int buf, unsigned int count) {
   ...
   // кэшируем порт и пин esp для быстрого включения в прерывании
   esp_io_port = FastIOPortMap[STM_PORT(digitalPinToPinName(MKS_WIFI_IO4))];
   esp_io_en_pin = _BV32(STM_PIN(digitalPinToPinName(MKS_WIFI_IO4)));

   USART1->CR1 = USART_CR1_UE;
   USART1->CR1 = USART_CR1_TE | USART_CR1_UE;
   USART1->BRR = (uint32_t)(84000000+1958400/2)/1958400; // 1958400 - бодрейт, 84000000 - F_CPU/2
   USART1->CR2 = 0;
...
extern "C" void DMA2_Stream5_IRQHandler(void) {
      ...
      if(buffer_ready > 0) { 
         //остановить передачу от esp, то же, что и WRITE(MKS_WIFI_IO4, HIGH);
         esp_io_port->BSRR = esp_io_en_pin;
         dma_stopped = 1;
      };
