...
void mks_wifi_out_add(uint8_t *data, uint32_t size){
	...
	while (size--){
		if(*data == 0x0a){
			...
			if(packet_size == 0){
				MKS_WIFI_DEBUG("[ERROR] Build packet failed");
				line_index=0;
				memset(mks_out_buffer,0,MKS_OUT_BUFF_SIZE);
				break;
			}
		...
		if(line_index >= MKS_OUT_BUFF_SIZE){
			MKS_WIFI_DEBUG("[ERROR] Max line size");
			line_index=0;
			memset(mks_out_buffer,0,MKS_OUT_BUFF_SIZE);
			break;
		}
...
uint8_t mks_wifi_input(uint8_t data){
	...
	// 	MKS_WIFI_DEBUG("No input while printing");
	// 	return 1;
	// }	
	...
	if(data == ESP_PROTOC_HEAD){
		...
		MKS_WIFI_DEBUG("Byte not in packet %0X",data);
		return 1;
	}
	...
	if(packet_index == 3){
		...
		if(payload_size > ESP_PACKET_DATA_MAX_SIZE){
			MKS_WIFI_DEBUG("[ERROR] Payload size too big");
			packet_start_flag=0;
			packet_index=0;
			memset((uint8_t*)mks_in_buffer,0,MKS_IN_BUFF_SIZE);
			return 1;
	...
	if( (packet_index >= (payload_size+4)) || (packet_index >= ESP_PACKET_DATA_MAX_SIZE) ){
		...
		if(mks_wifi_check_packet((uint8_t *)mks_in_buffer)){
			MKS_WIFI_DEBUG("[ERROR] Packet check failed");
			packet_start_flag=0;
			packet_index=0;
			return 1;
		}
		...
		if(!get_packet_from_esp){
			MKS_WIFI_DEBUG("Fisrt packet from ESP, send config");
...
uint8_t mks_wifi_check_packet(uint8_t *in_data){
	...
	if(in_data[0] != ESP_PROTOC_HEAD){
		MKS_WIFI_DEBUG("[ERROR] Packet head mismatch");
		return 1;
	}
	...
	if(payload_size > ESP_PACKET_DATA_MAX_SIZE){
		MKS_WIFI_DEBUG("[ERROR] Payload size mismatch");
		return 1;
	}
	...
	if(in_data[tail_index] != ESP_PROTOC_TAIL ){
		MKS_WIFI_DEBUG("[ERROR] Packet tail mismatch");
		return 1;
	}
			...
			if(packet->data[6] == ESP_NET_WIFI_CONNECTED){
				if(show_ip_once==0){
					...
					if(packet->data[8] < 100){
						...
						MKS_WIFI_DEBUG("Network NAME too long");
					}
				}
				MKS_WIFI_DEBUG("[Net] connected, IP: %s", str);
		...
		case ESP_TYPE_GCODE:
				...
				for(uint32_t i=0; i<packet->dataLen; i++){
					...
					if(packet->data[i] != 0x0A){
						...
						GCodeQueue::ring_buffer.enqueue((const char *)gcode_cmd, false OPTARG(HAS_MULTI_SERIAL, MKS_WIFI_SERIAL_NUM));
						cmd_index = 0;
						memset(gcode_cmd,0,50);
					}
				}
...
void mks_wifi_out_add(uint8_t *data, uint32_t size){
	while(size--){
		MSERIAL(MKS_WIFI_SERIAL_NUM).write(*data++);
	}
	return;
};
